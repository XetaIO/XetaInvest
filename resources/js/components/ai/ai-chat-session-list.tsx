...
export function AiChatSessionList({ sessions, activeId, onSelect, onCreate, onDelete }: AiChatSessionListProps) {
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-1">
                {sessions.length === 0 ? (
                    <p className="p-2 text-xs text-muted-foreground">Aucune conversation.</p>
                ) : (
                    sessions.map((s) => (
                        <div
                            key={s.id}
