...
import { deltaToneClass, formatDate, formatEur, formatNative, formatNumber, formatPercent } from '@/lib/format';
import { cn } from '@/lib/utils';
import { destroy as destroyPosition } from '@/routes/positions';
import { destroy as destroyTransaction } from '@/routes/transactions';
import type { PositionKpis, TransactionTypeOption } from '@/types';
                            ...
                            <div className="flex min-w-0 items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className="font-semibold">{position.instrument.name}</span>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        ...
                                        {position.lines.map((line) => (
                                            <tr key={line.transaction_id} className="border-t tabular-nums">
                                                <td className="px-3 py-2">{formatDate(line.executed_at)}</td>
                                                <td className="px-3 py-2 text-right">
                                                    {formatNumber(line.original_quantity)}
                                                </td>
                                                <td className="px-3 py-2 text-right">
                                                    {formatNumber(line.remaining_quantity)}
                                                </td>
                                                <td className="px-3 py-2 text-right">
                                                    {formatNative(line.unit_price, position.currency)}
                                                </td>
                                                <td className="px-3 py-2 text-right">
                                                    {formatNative(line.invested_native, position.currency)}
                                                </td>
                                                <td className="px-3 py-2 text-right">
                                                    {formatNative(line.current_value_native, position.currency)}
                                                </td>
                                                <td
                                                    className={cn(
                                                        'px-3 py-2 text-right',
                                                        deltaToneClass(line.pnl_native),
                                                    )}
                                                >
                                                    {formatNative(line.pnl_native, position.currency)}
                                                </td>
                                                <td className="px-3 py-2 text-right">
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: number }) {
    return (
        <div>
            ...
            <div className={cn('tabular-nums', tone !== undefined && deltaToneClass(tone))}>{value}</div>
        </div>
    );
}
