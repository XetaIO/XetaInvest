    ...
    return (
        <li className="flex items-center gap-3 border-b px-3 py-2 last:border-b-0">
            <span
                ...
                style={{ backgroundColor: visible ? color : 'transparent', borderColor: color, borderWidth: 2 }}
                aria-hidden
            />
            ...
            <div className="min-w-0 flex-1">
                ...
                {item.instrument.name && (
                    <p className="truncate text-xs text-muted-foreground">{item.instrument.name}</p>
                )}
            </div>
            ...
            <div className="text-right">
                {price ? (
                    <>
                        ...
                        <div
                            className={`flex items-center justify-end gap-1 text-xs tabular-nums ${isUp ? 'text-emerald-500' : 'text-rose-500'
                                }`}
                        >
            ...
            <Button
                ...
                title={visible ? 'Masquer du graphique' : 'Afficher sur le graphique'}
            >
                {visible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </Button>

            <Button type="button" variant="ghost" size="icon" onClick={remove} title="Retirer">
